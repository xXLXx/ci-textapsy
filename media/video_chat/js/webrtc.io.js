...
(function() {
  ...
  rtc.connect = function(server, room) {
    ...
    rtc._socket = new WebSocket('wss://devchat.psychic-contact.com:8888');
    ...
    rtc._socket.onopen = function() {
      ...
      rtc.on('remove_peer_connected', function(data) {
		  console.log('remove_peer_connected');
      ...
      rtc.on('receive_answer', function(data) {console.log('receive_answer');
  ...
  rtc.sendOffers = function() {
	...
	if(rtc.connections.length == 0){
		Chat.manager.room.send.vcall_accepted();
	}
	//\end
  };
  ...
  rtc.createPeerConnection = function(id) {
	...
	pc.onremovestream = function(event) {
		console.log('remove stream callback');
	  rtc.fire('remove remote stream', event.stream, id);
	};
	
	pc.onCallListener = function(callerId, extention){
		console.log('call listnerwe');
	}
  ...
  rtc.receiveOffer = function(socketId, sdp) {console.log('receive offer');
    var pc = rtc.peerConnections[socketId];
  ...
  rtc.receiveAnswer = function(socketId, sdp) {console.log('receive answer');
    var pc = rtc.peerConnections[socketId];
    pc.setRemoteDescription(new nativeRTCSessionDescription(sdp));
  };


  rtc.createStream = function(opt, onSuccess, onFail) {console.log('createstream');
    var options;
  ...
  rtc.addStreams = function() {console.log('Stream Length : '+rtc.streams.length);
    for (var i = 0; i < rtc.streams.length; i++) {
      var stream = rtc.streams[i];
  ...
  rtc.removeStreams = function() {	
	...
	for (var i = 0; i < rtc.streams.length; i++) {
		  ...
		  for (var elem in rtc.connections){
				delete rtc.connections[elem];
		  }
			
			
		 for (var chn in rtc.dataChannels){
				delete rtc.dataChannels[chn];
		}
			
		  stream.stop();		  
		}
		
		console.log('data cheannels');
		  console.log(rtc.peerConnections);
		  rtc.peerConnections = {};
		  rtc.connections = [];
		rtc.streams.pop();
		
		var videoTrack = vStream.getVideoTracks();
 
			if (videoTrack.length > 0) {		 
				vStream.removeTrack(videoTrack[0]);
			 
				var video = document.querySelector('video');
				video.src = window.URL.createObjectURL(vStream);		 
			}
			var element = document.getElementsByTagName("video");
			for (index = element.length - 1; index >= 0; index--) {
				if(index == 0) element[0].setAttribute('src','');
				else 
